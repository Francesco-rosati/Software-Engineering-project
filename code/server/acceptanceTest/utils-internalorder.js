...
function testEditInternalOrder(agent, newState, expCode){
    describe(' put /api/internalOrders/:id', function(){
		...
		.send({"newState":newState, "products": 
[{"SkuID":res.body[0].products[0].SKUId,"RFID":"12345678901234567890123456789016"},{"SkuID":res.body[0].products[0].SKUId,"RFID":"12345678901234567890123456789038"}]})
                .then(function(res2){
                    res2.should.have.status(expCode);
                    done();
                }).catch(err=>done(err));
